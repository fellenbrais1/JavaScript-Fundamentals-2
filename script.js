...
function logger(input) {
  console.log(
    `This is a call from the logger() function, the input is ${input}.`
  );
}
...
function foodProcessor(food1, food2) {
  ...
  let opinion =
    food1 === "worm" || food2 === "worm" ? "Disgusting!" : "Delicious!";
  console.log(
    `The food processor combines ${food1} and ${food2} into ${grammar} ${food1} and ${food2} smoothie! ${opinion}`
  );
}
...
function juiceMaker() {
  ...
  console.log(
    `You wanted a juice made of ${applesWanted} apples, ${orangesWanted} oranges, and ${bananasWanted} bananas.`
  );
...
console.log(
  `The complete name is ${person.firstName}, ${person.middleName}, ${person.lastName}`
);
...
console.log(
  `This ${car.type} is an ${car.model} built in ${car.yearOfManufacture}. It ${isNotIs} a pro model`
);
...
let choice = prompt(
  "What information would you like to see about the car? type, model, proModel, yearOfManufacture"
);
...
if (car[choice] != undefined) {
  ...
  console.log(
    "There is no such field in the car object, please reload and try again."
  );
}
...
const judy = {
  ...
  getSummary: function () {
    this.summary = `${this.firstName} is a ${this.calcAge7(
      this.birthYear
    )} year-old ${
      this.job
    }, and she ${this.grammarLicense()} a driver's license.`;
    return this.summary;
  },
};
...
function coinFlipper(sampleSize = 10) {
  ...
  console.log(
    `Out of ${sampleSize} repetitions, we saw ${heads} instances of heads (${headsPerc}%) and ${tails} instance of tails (${tailsPerc}%)`
  );
}
...
console.log(
  `Average of bills is ${avBills}, the average of tips is ${avTips}, and the average of the totals is ${avTotals}`
);
